...
public class testCase_API_02 {
    ...
    public void TestCase02() {
        ...
        Assert.assertEquals(searchCityResStatusCode, 200,"Failed to validate that the search city response status code is 200");
        String resBody = body.asString();
        JsonPath jpath = new JsonPath(resBody);
        String description = jpath.getString("description");
        Assert.assertEquals(resBody.contains("description"), true,"Failed to validate that description field is present in the search response body");
        Assert.assertEquals(description, "[100+ Places]","Failed to validate the contents of search response description field");
        int searchResSize = jpath.getInt("size()");
        Assert.assertEquals(searchResSize, 1, "Failed to validate that search size response is 1");
        response.then().assertThat().body(JsonSchemaValidator.matchesJsonSchema(new File("/home/crio-user/workspace/sarojkumar-criodo-ME_API_TESTING_PROJECT/app/src/test/resources/jsonSchema.json")));
    }
}
